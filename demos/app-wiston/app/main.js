const { 
    app, BrowserWindow, Tray, nativeImage, Menu, 
    shell, ipcMain, dialog, 
    clipboard, systemPreferences 
} = require('electron');
...
//created by AI
// 仿bili窗口创建函数
function createBilibiliWindow() {
    console.log('Creating Bilibili window...');

    if (bilibiliWindow && !bilibiliWindow.isDestroyed()) {
        console.log('Bilibili window already exists, focusing...');
    ...
    try {
        ...
        console.log('Bilibili window created, loading bilibili.html...');
        bilibiliWindow.loadFile(path.join(__dirname, 'bilibili.html'));

        bilibiliWindow.on('closed', function () {
            console.log('Bilibili window closed');
            bilibiliWindow = null;
        });

        bilibiliWindow.webContents.on('did-finish-load', function () {
            console.log('Bilibili window content loaded');
        });

        bilibiliWindow.webContents.on('did-fail-load', function (event, errorCode, errorDescription) {
            console.error('Bilibili window failed to load:', errorCode, errorDescription);
        });
    } catch (error) {
        console.error('Failed to create Bilibili window:', error);
    }
}

// 仿bili窗口IPC处理
ipcMain.on('open-bilibili', function () {
    console.log('IPC: open-bilibili received');
    console.log('mainWindow exists:', !!mainWindow);
    if (!mainWindow) {
        console.error('mainWindow is not created yet');
        return;
    }
    createBilibiliWindow();
});
//created by AI
// winston三方库调用窗口创建函数
function createWinstonWindow() {
    console.log('Creating Winston window...');

    if (winstonWindow && !winstonWindow.isDestroyed()) {
        console.log('Winston window already exists, focusing...');
        winstonWindow.focus();
        return;
    }
    ...
    try {
        winstonWindow = new BrowserWindow({
            width: 1000,
            height: 700,
            parent: mainWindow,
            title: 'Winston日志管理',
        ...
        console.log('Winston window created, loading winston.html...');
        winstonWindow.loadFile(path.join(__dirname, 'winston.html'));

        winstonWindow.on('closed', function () {
            console.log('Winston window closed');
            winstonWindow = null;
        });

        winstonWindow.webContents.on('did-finish-load', function () {
            console.log('Winston window content loaded');
        });

        winstonWindow.webContents.on('did-fail-load', function (event, errorCode, errorDescription) {
            console.error('Winston window failed to load:', errorCode, errorDescription);
        });
    } catch (error) {
        console.error('Failed to create Winston window:', error);
    }
}

// winston三方库调用窗口IPC处理
ipcMain.on('open-winston', function () {
    console.log('IPC: open-winston received');
    console.log('mainWindow exists:', !!mainWindow);
    if (!mainWindow) {
        console.error('mainWindow is not created yet');
        return;
...
// async三方库调用窗口变量
let asyncWindow = null;

// async三方库调用窗口创建函数
function createAsyncWindow() {
    console.log('Creating Async window...');

    if (asyncWindow && !asyncWindow.isDestroyed()) {
        console.log('Async window already exists, focusing...');
        asyncWindow.focus();
        return;
    }
    ...
    try {
        ...
        console.log('Async window created, loading async.html...');
        asyncWindow.loadFile(path.join(__dirname, 'async.html'));

        asyncWindow.on('closed', function () {
            console.log('Async window closed');
            asyncWindow = null;
        });

        asyncWindow.webContents.on('did-finish-load', function () {
            console.log('Async window content loaded');
        });

        asyncWindow.webContents.on('did-fail-load', function (event, errorCode, errorDescription) {
            console.error('Async window failed to load:', errorCode, errorDescription);
        });
    } catch (error) {
        console.error('Failed to create Async window:', error);
    }
}

// async三方库调用窗口IPC处理
ipcMain.on('open-async', function () {
    console.log('IPC: open-async received');
    console.log('mainWindow exists:', !!mainWindow);
    if (!mainWindow) {
        console.error('mainWindow is not created yet');
        return;
